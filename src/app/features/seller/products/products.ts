...
//       if (params['openAdd'] === 'true') this.openAdd();
//     });
//   }
...
export class Products implements OnInit, OnDestroy {
  ...
  private refreshHandler = () => this.loadProducts();

  ngOnInit() {
    this.loadProducts();
    this.loadCategories();
    window.addEventListener('products:refresh', this.refreshHandler);
    this.route.queryParams.subscribe((params) => {
      if (params['openAdd'] === 'true') this.modalSvc.open('add');
    });
  }

  ngOnDestroy() {
    window.removeEventListener('products:refresh', this.refreshHandler);
  }
  ...
  loadCategories() {
    this.categorySvc.getAll().subscribe({
      next: (res) => this.modalSvc.setCategories(res.data),
    });
  }
}